        ...
        { officeClients.map(client => {
          ...
          return (
            <div className={'client-grid-article client-grid-article--' + articleNumber} key={client.fields.slug}>
              <Visible xs sm>
                {client?.fields?.mainImageMobile &&<img src={extractMediaAssetSrc(client.fields.mainImageMobile)} />}
              </Visible>
              <Visible md lg xl xxl>
                {client?.fields?.mainImageDesktop && <img src={extractMediaAssetSrc(client.fields.mainImageDesktop)} />}
              </Visible>
            </div>
          )
        })}
